...
final class OfflineStore {
    ...
    static int drain(Context ctx) {
        ...
        String server = HealthServerConfig.resolveOrActive(ctx);
    ...
    static boolean fetchBootstrap(Context ctx) {
        String server = HealthServerConfig.resolveOrActive(ctx);  // IO 线程调用，允许探测
        String token = prefs(ctx).getString("ingest_token", "");
        if (server.isEmpty() || token.isEmpty()) {
            return false;
        }
    ...
    private static boolean postJson(String url, String token, String json) {
        ...
        return HealthHttpPost.postJson(TAG, url, token, json, 8000, 15000);
    }
}
